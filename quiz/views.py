...
def create_quiz_repeat_and_learn(request):
    ...
    if number_of_needed_words > 2:
        random_words = EnglishWord.objects.exclude(id__in=user_word_ids).order_by('?')[:number_of_needed_words]
    else:
        random_words = EnglishWord.objects.exclude(id__in=user_word_ids).order_by('?')[:2]
